// +FHDR------------------------------------------------------------
//                 Copyright (c) 2026 Wolley Inc.
//                       ALL RIGHTS RESERVED
// -----------------------------------------------------------------
// Filename      : cdp_icvt_ref_tb.v
// Author        : Claude
// Created On    : 2026/04/05
// -----------------------------------------------------------------
// Description:
//   Testbench for CDP ICVT Reference Model
// +FHDR------------------------------------------------------------

`timescale 1ns / 1ps

module cdp_icvt_ref_tb;

    reg nvdla_core_clk;
    reg nvdla_core_rstn;

    reg chn_data_in_vld;
    wire chn_data_in_rdy;
    reg [15:0] chn_data_in;

    reg [1:0] cfg_precision;
    reg [15:0] cfg_alu_in;
    reg [15:0] cfg_mul_in;
    reg [7:0] cfg_truncate;

    wire chn_data_out_vld;
    reg chn_data_out_rdy;
    wire [15:0] chn_data_out;

    // DUT
    cdp_icvt_ref #(
        .DATA_IN_WIDTH(16),
        .ALU_IN_WIDTH(16),
        .MUL_IN_WIDTH(16),
        .TRUNCATE_WIDTH(8),
        .DATA_OUT_WIDTH(16)
    ) dut (
        .nvdla_core_clk(nvdla_core_clk),
        .nvdla_core_rstn(nvdla_core_rstn),
        .chn_data_in_vld(chn_data_in_vld),
        .chn_data_in_rdy(chn_data_in_rdy),
        .chn_data_in(chn_data_in),
        .cfg_precision(cfg_precision),
        .cfg_alu_in(cfg_alu_in),
        .cfg_mul_in(cfg_mul_in),
        .cfg_truncate(cfg_truncate),
        .chn_data_out_vld(chn_data_out_vld),
        .chn_data_out_rdy(chn_data_out_rdy),
        .chn_data_out(chn_data_out)
    );

    // Clock
    initial begin
        nvdla_core_clk = 0;
        forever #5 nvdla_core_clk = ~nvdla_core_clk;
    end

    integer test_count;
    integer pass_count;

    // Expected calculation
    wire [15:0] expected_alu;
    wire [15:0] expected_mul;
    wire [15:0] expected_trt;
    assign expected_alu = chn_data_in - cfg_alu_in;
    assign expected_mul = expected_alu * cfg_mul_in;
    assign expected_trt = expected_mul >> cfg_truncate;

    initial begin
        test_count = 0;
        pass_count = 0;
        chn_data_in_vld = 0;
        chn_data_out_rdy = 1;
        cfg_precision = 2'b01;  // INT16
        cfg_alu_in = 16'd10;
        cfg_mul_in = 16'd2;
        cfg_truncate = 8'd4;

        nvdla_core_rstn = 0;
        #100;
        nvdla_core_rstn = 1;
        #50;

        // Test 1: Basic INT16 operation
        test_count = test_count + 1;
        chn_data_in = 16'd100;
        chn_data_in_vld = 1;
        @(posedge nvdla_core_clk);
        while (!chn_data_in_rdy) @(posedge nvdla_core_clk);
        chn_data_in_vld = 0;

        // Wait for output - chn_data_out_vld is combinational, check after clock edge
        while (!chn_data_out_vld) @(posedge nvdla_core_clk);
        if (chn_data_out == expected_trt) begin
            $display("PASS: Test 1 - ICVT result=%h (expected %h)", chn_data_out, expected_trt);
            pass_count = pass_count + 1;
        end else begin
            $display("FAIL: Test 1 - ICVT result=%h (expected %h)", chn_data_out, expected_trt);
        end

        // Test 2: Another value
        test_count = test_count + 1;
        chn_data_in = 16'd200;
        chn_data_in_vld = 1;
        @(posedge nvdla_core_clk);
        while (!chn_data_in_rdy) @(posedge nvdla_core_clk);
        chn_data_in_vld = 0;

        // Wait for output - chn_data_out_vld is combinational, check after clock edge
        while (!chn_data_out_vld) @(posedge nvdla_core_clk);
        if (chn_data_out == expected_trt) begin
            $display("PASS: Test 2 - ICVT result=%h (expected %h)", chn_data_out, expected_trt);
            pass_count = pass_count + 1;
        end else begin
            $display("FAIL: Test 2 - ICVT result=%h (expected %h)", chn_data_out, expected_trt);
        end

        $display("========================================");
        $display("CDP ICVT Reference Model Test Summary:");
        $display("  Total: %0d", test_count);
        $display("  Pass:  %0d", pass_count);
        $display("  Fail:  %0d", test_count - pass_count);
        $display("========================================");

        $finish;
    end

endmodule