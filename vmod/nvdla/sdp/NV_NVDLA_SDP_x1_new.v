...
//                       ALL RIGHTS RESERVED
// -----------------------------------------------------------------
// Filename      : NV_NVDLA_SDP_x1_new.v
// Author        : Wolley RTL Team
// Author Email  : rtl@wolley.com
// Created On    : 2026/04/04
// -----------------------------------------------------------------
// Description:
// X1 Processing Path (BS - Block Select)
...
module NV_NVDLA_SDP_x1_new (
  ...
  ,cfg_x1_alu_algo                 //|< i
  ,cfg_x1_alu_src                  //|< i
  ,cfg_x1_alu_shift_value           //|< i
  ,cfg_x1_alu_operand              //|< i
  ,cfg_x1_mul_bypass               //|< i
  ,cfg_x1_mul_src                  //|< i
  ,cfg_x1_mul_shift_value          //|< i
  ,cfg_x1_mul_operand              //|< i
  ,cfg_x1_mul_prelu                //|< i
  ,cfg_x1_relu_bypass              //|< i
  ,cfg_x1_nan_to_zero              //|< i
  ,cfg_x1_proc_precision           //|< i
  // Output
  ,x1_out_data                     //|> o
  ,x1_out_valid                    //|> o
  ,x1_out_ready                    //|< i
...
wire  [255:0] x1_in_data;  // 16 elements x 16 bits
wire x1_in_valid;
wire x1_in_ready;

// ALU operand
wire  [255:0] x1_alu_op;
wire x1_alu_op_valid;

// MUL operand
wire  [255:0] x1_mul_op;
wire x1_mul_op_valid;

// Configuration
wire cfg_x1_bypass;
wire cfg_x1_alu_bypass;
wire [1:0] cfg_x1_alu_algo;
wire cfg_x1_alu_src;
wire [5:0] cfg_x1_alu_shift_value;
wire [15:0] cfg_x1_alu_operand;
wire cfg_x1_mul_bypass;
wire cfg_x1_mul_src;
wire [7:0] cfg_x1_mul_shift_value;
wire [15:0] cfg_x1_mul_operand;
wire cfg_x1_mul_prelu;
wire cfg_x1_relu_bypass;
wire cfg_x1_nan_to_zero;
wire [1:0] cfg_x1_proc_precision;
...
localparam [1:0] STATE_ALU    = 2'd1;
localparam [1:0] STATE_MUL    = 2'd2;
localparam [1:0] STATE_TRT    = 2'd3;
localparam [1:0] STATE_RELU   = 2'd3;
...
always @(posedge nvdla_core_clk or negedge nvdla_core_rstn) begin
  ...
  end else if (x1_in_valid && x1_in_ready) begin
    alu_in_data_r <= x1_in_data;
  end
end
...
always @(posedge nvdla_core_clk or negedge nvdla_core_rstn) begin
  if (!nvdla_core_rstn) begin
    ...
    if (cfg_x1_alu_src == 1'b0) begin
      // ALU operand from register
      alu_op_r <= {16{cfg_x1_alu_operand}};
      alu_op_valid_r <= 1'b1;
...
generate
  for (i = 0; i < 16; i = i + 1) begin : gen_alu
    ...
    assign shifted_op = alu_op_elem << cfg_x1_alu_shift_value[5:1];

    // ALU operation
    always @* begin
      if (cfg_x1_proc_precision == PRECISION_FP16) begin
        // FP16 ALU - placeholder for actual FP16 ALU
        case (cfg_x1_alu_algo)
...
assign alu_result = cfg_x1_alu_bypass ? alu_in_data_r : alu_out_r;
...
always @(posedge nvdla_core_clk or negedge nvdla_core_rstn) begin
  if (!nvdla_core_rstn) begin
    ...
    if (cfg_x1_mul_src == 1'b0) begin
      // MUL operand from register
      mul_op_r <= {16{cfg_x1_mul_operand}};
      mul_op_valid_r <= 1'b1;
...
generate
  for (i = 0; i < 16; i = i + 1) begin : gen_mul
    ...
    wire mul_bypass_trt;
    assign mul_bypass_trt = cfg_x1_mul_prelu && sign_bit;
...
always @(posedge nvdla_core_clk or negedge nvdla_core_rstn) begin
  if (!nvdla_core_rstn) begin
    ...
    if (cfg_x1_mul_shift_value[7]) begin
      // Right shift
      trt_out_r <= mul_out_r >> cfg_x1_mul_shift_value[6:0];
    end else begin
      trt_out_r <= mul_out_r;
    end
  end
...
generate
  for (i = 0; i < 16; i = i + 1) begin : gen_relu
    ...
    wire [15:0] relu_out_elem;

    assign relu_in_elem = trt_out_r[i*16 +: 16];

    always @* begin
      if (cfg_x1_relu_bypass) begin
...
// Output assignment
//============================================================================
assign x1_out_data = relu_out_r;
assign x1_out_valid = relu_valid_r;

// Ready signal - always ready when not bypassed
assign x1_in_ready = 1'b1;  // Always accept new input when op_en is set

endmodule // NV_NVDLA_SDP_x1_new