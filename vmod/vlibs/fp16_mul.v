...
// -----------------------------------------------------------------
// Description:
//   3-stage floating-point multiplier for fp16 (1 sign + 5 expo + 10 mantissa)
//   Implements FpMul<5,10> algorithm from nvdla_float.h
...
module fp16_mul (
    ...
    localparam K_P_MANT_WIDTH = 2 * MANT_WIDTH + 1;  // 21 (product mantissa width)
    ...
    // Mantissa multiplication: 11-bit * 11-bit = 22-bit product
    ...
    wire [2*MANT_WIDTH+1:0] product_full;  // 22-bit product
    assign product_full = a_mant_p1 * b_mant_p1;

    // Exponent calculation and normalization
    wire [EXPO_WIDTH:0] sum_expo;  // 6-bit to avoid overflow
    ...
    assign product_top_bit = product_full[K_P_MANT_WIDTH-1];  // bit 20 (MSB of 21-bit product)
    ...
    wire [MANT_WIDTH:0] mant_to_round;  // 11 bits: 1 implicit + 10 explicit
    wire guard_bit;
    wire round_bit;
    wire sticky_bit;
    wire lsb_bit;
    ...
    assign mant_to_round = s2_product_mant_q[K_P_MANT_WIDTH-2 -: MANT_WIDTH+1];  // bits [K_P_MANT_WIDTH-2:MANT_WIDTH-1]
    ...
    assign lsb_bit = mant_to_round[0];  // LSB of the 10-bit mantissa
